...
export default function Auctions(props){
  ...
  const auctionState = (auction)=>{
    return (
      <span>
          {currentDate < new Date(auction.bidStart) && `La Oferta comienza a las ${new Date(auction.bidStart).toLocaleString()}`}
          {currentDate > new Date(auction.bidStart) && currentDate < new Date(auction.bidEnd) && <>{`La Oferta en vivo | ${auction.bids.length} ofertas |`} {showTimeLeft(new Date(auction.bidEnd))}</>}
          {currentDate > new Date(auction.bidEnd) && `Oferta finalizada| ${auction.bids.length} ofertas `} 
          {currentDate > new Date(auction.bidStart) && auction.bids.length> 0 && ` | Última oferta: $ ${auction.bids[0].bid}`}
      </span>
    )
  }
    return (
